...
import '../components/FriendsModal.css'; // Reusing the same CSS for now
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'var(--bg-primary)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
      {selectedFriendProfile ? (
        <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
          {selectedFriendProfile.customBg && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, opacity: 0.2, zIndex: 0, pointerEvents: 'none', overflow: 'hidden' }}>
              {selectedFriendProfile.customBg.includes('/video/upload/') || selectedFriendProfile.customBg.match(/\.(mp4|webm|mov)$/i) ? (
                <video src={selectedFriendProfile.customBg} autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <div style={{ width: '100%', height: '100%', backgroundImage: `url(${selectedFriendProfile.customBg})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
              )}
            </div>
          )}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '20px', paddingTop: 'calc(20px + env(safe-area-inset-top))', display: 'flex', alignItems: 'center' }}>
            <button onClick={() => navigate(-1)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '50%', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', cursor: 'pointer' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
            </button>
            <h3 style={{ flex: 1, textAlign: 'center', margin: 0, color: '#fff', fontSize: '18px', marginRight: '40px' }}>Bạn bè</h3>
          </div>

          <div className="friends-tabs" style={{ padding: '0 20px', marginBottom: '16px' }}>
            <div className={`friends-tab ${activeTab === 'friends' ? 'active' : ''}`} onClick={() => setActiveTab('friends')}>Danh sách</div>
            <div className={`friends-tab ${activeTab === 'search' ? 'active' : ''}`} onClick={() => setActiveTab('search')}>Tìm bạn</div>
            <div className={`friends-tab ${activeTab === 'requests' ? 'active' : ''}`} onClick={() => setActiveTab('requests')}>
